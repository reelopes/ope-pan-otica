...
foreach ($produto as $linha) {
    ...
    $this->table->add_row($linha->referencia, $linha->nome, $linha->preco_custo, $linha->preco_venda, $linha->quantidade, $linha->status, anchor("produto/visualiza/$linha->id_produto", '<center><img src="..\public/img/search.png" width="23"/></center>'),anchor("produto/update/$linha->id_produto", '<center><img src="..\public/img/edit.png" width="23"/></center>'),'<center><p onClick="if (! confirm(\'Tem certeza que deseja excluir o produto abaixo? \n\n Referência: '.$linha->referencia.'\n Nome: '.$linha->nome.'\n Quantidade: '.$linha->quantidade.'\')) { return false; }">'.anchor('produto/delete/'.$linha->id_produto,'<img src="..\public/img/delete.png" width="23"/>').'</p></center>');
